...
mod shell_tests {
    ...
    fn test_detect_shell_config() {
        ...
        assert!(
            config_path.exists() || config_path.parent().map_or(false, |p| p.exists()),
            "Config path or its parent directory should exist"
        );
        assert!(
            alias_line.contains("g") && alias_line.contains("git"),
    ...
    fn test_add_to_shell_config() -> anyhow::Result<()> {
        ...
        assert!(
            ...
            "Config should contain the alias"
        );
        assert!(
            content.contains("Added by Super Snoofer"),
            "Config should have the comment"
        );
