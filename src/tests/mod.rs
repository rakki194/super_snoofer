#[cfg(test)]
use std::{fs, io::Write, process::Command, sync::Once};
...
pub mod tests {
    ...
    fn test_command_line_correction() -> Result<()> {
        ...
        // Initialize a fresh cache
        {
            ...
            cache.learn_correction(
                "docker run --detetch --naem container",
    ...
    fn test_fix_command_line_integrated() -> Result<()> {
        ...
        let fixed =
            crate::command::fix_command_line("gti push --globel --al", find_similar, &patterns);
        assert_eq!(
            fixed,
    ...
    fn test_check_command_feature() -> Result<()> {
        ...
        assert!(
            output.contains("git status --all\n"),
    ...
    fn test_record_correction_feature() -> Result<()> {
        ...
        // Verify the correction was recorded
        {
            ...
            assert_eq!(
                entry.correction, "git",
                "Correction should be recorded in history"
            );
        }
    ...
    fn test_record_valid_command_feature() -> Result<()> {
        ...
        // Verify the command was recorded correctly
        {
            ...
            assert!(
                has_git,
                "The base command 'git' should be recorded in correction frequencies"
            );

            // The command should be added to the commands list
            assert!(
                cache.contains("git"),
                "The command 'git' should be added to known commands"
            );
        }
    ...
    fn test_real_time_suggestions() -> Result<()> {
        ...
        // Test suggestions
        {
            ...
            assert_eq!(
                suggestion,
                Some("git status --verbose".to_string()),
    ...
    fn test_suggestion_command_integration() -> Result<()> {
        ...
        assert_eq!(
            ...
            "Should suggest 'git status' for 'git s'"
        );
        ...
        assert_eq!(
            suggestion, "git status",
            "Should suggest 'git status' for typo 'gti s'"
        );
