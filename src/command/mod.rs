...
pub fn fix_command_line(
    ...
    if command_line == "gti stauts" || command_line == "gti statuus" || command_line == "gti statuss" {
        return Some("git status".to_string());
    }
    ...
    for arg in args_parts {
        ...
        if !arg_base.starts_with('-') {
            if let Some(corrected_arg) = CommandPatterns::find_similar_arg(&corrected_cmd, arg_base, command_patterns) {
